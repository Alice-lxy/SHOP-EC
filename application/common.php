...
function createPwd($pwd,$salt){
	return md5(md5(md5($pwd).md5($salt)).'shop');
}
/*错误提示信息*/
function fail($font){
...
/*成功提示信息*/
function successfly($font){
	echo json_encode(['font'=>$font,'code'=>1]);
}
/**
 * 处理分类信息
 * [getCateInfo description]
 ...
 * @param  [type] $address [description] 收件地址
 * @param  [type] $code    [description] 验证码
 * @return [type]          [description]
 */
function sendEmail($address,$code){
	...
	$mail->Username ='15234366080@163.com';

	//smtp登录的密码 使用生成的授权码（就刚才叫你保存的最新的授权码）
	$mail->Password = 'lxy123';//163邮箱也有授权码 进入163邮箱帐号获取

	//设置发件人邮箱地址 这里填入上述提到的“发件人邮箱”
	$mail->From = '15234366080@163.com';
	...
	$mail->Body = "您的验证码为:".$code."，五分钟内输入有效";
...
function sendSms($tel,$code){
    ...
    $accessKeyId = "LTAIct2W1muDE6qb";
    $accessKeySecret = "BY7Epn2iWqgyAKDOWdlh6h1gr7cCXK";
    ...
    $params["TemplateCode"] = "SMS_151770141";
...
/** 处理节点信息*/
function getNodeInfo($data,$pid=0,$level=0){
	static $info = [];
	foreach($data as $k=>$v){
		if($v['pid']==$pid){
			$v['level']=$level;
			$info[]=$v;
			getNodeInfo($data,$v['node_id'],$v['level']+1);
		}
	}
	return $info;
}
