	...
	$(function(){
		layui.use(['form','layer'],function(){
			...
			form.on('submit(*)', function(data){
			  	$.post(
			  		"<?php echo url('Admin/updatePwd'); ?>",
			  		data.field,
			  		function(result){
			  			layer.msg(result.font,{icon:result.code});
			  			if(result.code==1){
			  				location.href="<?php echo url('Login/quit'); ?>";
			  			}
			  		}
			  		,'json'
			  	);
