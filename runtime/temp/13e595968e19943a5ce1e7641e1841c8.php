...
<html lang="en">
  ...
  <div class="layui-body">
    ...
    <div style="padding: 15px; width: 635px">
    <h2 align="center">友链列表展示</h2>
...
<script>
	$(function(){
		layui.use(['table','layer'],function(){
			...
			table.render({
				elem: '#demo'
				,url: "<?php echo url('Friend/getFriendInfo'); ?>" //数据接口
				,page: true //开启分页
				,limit: 3   //限制条数
				,cols: [[ //表头
					{field: 'friend_id', title: 'ID', width:80, sort: true, fixed: 'left'}
					,{field: 'friend_name', title: '友链名称',edit: 'text', width:200}
					,{field: 'friend_url', title: '网址', width:200, sort: true}
			...
			table.on('edit(table_edit)', function(obj){ //注：edit是固定事件名，table_edit是table原始容器的属性 lay-filter="对应的值"
				var value = obj.value,//得到修改后的值
					friend_id = obj.data.friend_id,//得到所在行的数据
					field = obj.field;//得到编辑的字段名
				$.post(
					"<?php echo url('Friend/friendUpd'); ?>",
			...
			table.on('tool(table_edit)', function(obj){
				...
				if(obj.event === 'del'){
					...
					location.href="<?php echo url('Friend/friendUpdate'); ?>?friend_id="+data.friend_id;
				}
			});
